...
### Virtual DOM 을 Real DOM 으로 전환하기
...
- `tiny-react.js`
  ...
  - `appendChild()` 는 `HTMLElement` 에 자식 `element` 를 붙이는 함수이다.
  ...
  `VDOM` tree 를 매개변수로 받아 **`Real DOM tree` 를 생성**하여 `return` 하는 함수이다.
...
- `React.createElement`  
  jsx 를 분석해 Tree 구조의 VDOM 을 만드는 함수
